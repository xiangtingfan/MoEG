...
#
# 鏀瑰姩鐩爣锛?
# 1. 淇濈暀鍘熷 MoGE 涓诲共缁撴瀯锛?
# 2. 涓嶉澶栧姞鍏?shared emotion branch锛?
# 3. 灏嗗師鏉ョ殑 channel-level hard routing
#    鏀逛负 sample-style top-k sparse routing锛?
# 4. 姣忎釜 EEG 鏍锋湰鏍规嵁鑷韩椋庢牸閫夋嫨 top-k 涓?GCN experts锛?
# 5. router 涓嶄娇鐢?subject_id锛屽彧鏍规嵁 EEG 琛ㄥ緛鏈韩璺敱銆?
#########################################################
...
class SampleStyleTopKRouter(nn.Module):
    ...
    鏍稿績鎬濇兂锛?
    - 涓嶄娇鐢?subject_id锛?
    - 鏍规嵁 EEG 鏍锋湰鑷韩鐨勫叏灞€椋庢牸鐗瑰緛鐢熸垚 expert 鏉冮噸锛?
    - 姣忎釜鏍锋湰鍙縺娲?top-k 涓?expert锛?
    - 淇濈暀 MoE / GMoE 鐨?sparse expert 鎬濇兂銆?
    ...
    def forward(self, y):
        ...
        # 瀵规椂闂寸淮搴?T 鍜岄€氶亾/鑺傜偣缁村害 P 鍋氬钩鍧囨睜鍖?
        # 寰楀埌姣忎釜鏍锋湰鐨?EEG 椋庢牸鍚戦噺
        # style: [N, F]
...
class SampleStyleSparseMoeGCNTransformerUnit(nn.Module):
    ...
    涔熷氨鏄锛?
        涓嶆槸姣忎釜閫氶亾鍗曠嫭閫?expert锛?
        鑰屾槸姣忎釜鏍锋湰鏍规嵁鏁翠綋 EEG 椋庢牸閫夋嫨灏戦噺 expert銆?
    """
